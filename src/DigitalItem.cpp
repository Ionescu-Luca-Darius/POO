...
DigitalLoanPolicy DigitalItem::policy;

DigitalItem::DigitalItem()
    : LibraryItem(), fileFormat("EPUB"), fileSizeMB(0.0) {}

DigitalItem::DigitalItem(const std::string& title, const std::string& author,
                         int year, const std::string& fileFormat, double fileSizeMB)
    : LibraryItem(title, author, year),
      fileFormat(fileFormat), fileSizeMB(fileSizeMB)
{
    if (fileSizeMB < 0)
        throw InvalidItemException("File size cannot be negative.");
}

DigitalItem::DigitalItem(const DigitalItem& other)
    : LibraryItem(other),
      fileFormat(other.fileFormat), fileSizeMB(other.fileSizeMB) {}
...
void DigitalItem::checkOut() { checkedOut = true;  }
void DigitalItem::returnItem() { checkedOut = false; }
...
std::ostream& operator<<(std::ostream& os, const DigitalItem& d) {
    os << static_cast<const LibraryItem&>(d)
       << "  Format: " << d.fileFormat << "  Size: " << d.fileSizeMB << " MB";
    return os;
}
