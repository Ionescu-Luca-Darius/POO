...
ReferenceLoanPolicy Magazine::policy;

Magazine::Magazine()
    : LibraryItem(), issueNumber(0), category("General") {}
...
Magazine::Magazine(const Magazine& other)
    : LibraryItem(other),
      issueNumber(other.issueNumber), category(other.category) {}
...
const LoanPolicy& Magazine::getLoanPolicy() const { return policy;}

void Magazine::displayDetails() const {
    std::cout << *this << "\n"
              << "   Issue #" << issueNumber << "  Category: " << category
...
std::ostream& operator<<(std::ostream& os, const Magazine& m) {
    os << static_cast<const LibraryItem&>(m)
       << "  Issue #" << m.issueNumber << "  Category: " << m.category;
    return os;
}
